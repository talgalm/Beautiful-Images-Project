...
import { handleCreateImage, handleDeleteImage, handleGetAllImagesPaginated, handleUpdateImage } from '../../services/adminService';
...
const ImagesAdminPage = () => {
  ...
  const editCategoryDB = (item) => {
    handleUpdateImage("admin@gmail.com", item.id, item.imageName, editedCategory, item.imageData).then((response) => {
      setIsEdit(false);
      setEditIndex(-1)
      setTimeout(() => {
        fetchImages()
  ...
  const classifyImage = async (image) => {
    ...
    setSelectedCategory(topTags.flatMap(str => str.split(',').map(item => item.trim()))[0])
  ...
  const createImage = async () => {
    await handleCreateImage("admin@gmail.com" , imageName , selectedCategory, imageData).then((response) => {
      setShowAddModal(false);
      setTimeout(() => {
        fetchImages()
      }, 500);
  ...
  const deleteImage = async () => {
    await handleDeleteImage("admin@gmail.com", imageToDelete.id).then((response) => {
            ...
            <tbody className="thead-dark-body">
              {images.map((item, index) => (
                <tr key={index}>
                  ...
                  <td onClick={()=>openDeleteModal(item)}><img src={deleteIcon}/></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Modal.Body>
          <div>
            ...
            <div className='buttons'>
              <button className="btn btn-primary" disabled={!imageFile} onClick={generateCategory}>
              {t('generateCategory')} 
              </button>
              <button className="btn btn-primary" disabled={!imageFile} onClick={enterCategory}>
              {t('enterMan')} 
              </button>
            </div>
            {!loading ? (
              <div className='tagsDiv'>
                  {gen && <select  onChange={handleSelectChange} value={selectedCategory}>
                    {tags.map((tag, index) => (
            ...
            {input !== false && 
                <div className='sendDiv'>
                  <input placeholder={t('enterMan')} value={selectedCategory} onChange={setSelectedValueText}/>
                  <input placeholder={t('enterName')} value={imageName} onChange={(e) => setImageName(e.target.value)}/>
                  <button className="btn btn-primary" style={{backgroundColor:'red !important'}} onClick={createImage}>Save</button>
                </div>
            }
          </div>
        </Modal.Body>
      </Modal>
      <Modal show={showDeleteModal} onHide={()=>setShowDeleteModal(false)} size="m">
        <Modal.Header closeButton></Modal.Header>
        <Modal.Body>
          <div className='deleteContainer'>Are you sure you want to delete ?</div>
            <div className='imgDiv'>
              ...
              <div className='buttonsDiv'>
                <button className="btn btn-primary" onClick={deleteImage}>Delete Image</button>
