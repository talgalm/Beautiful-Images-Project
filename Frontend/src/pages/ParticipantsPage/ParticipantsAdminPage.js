...
const ParticipantsAdminPage = () => {
    ...
    setFilteredUsers(
      users.filter((user) => {
        return (
          ...
          user.age >= filter.ageRange[0] &&
          user.age <= filter.ageRange[1]
        );
      })
    );
  };
  ...
  const AgeDistributionChart = ({ filteredUsers }) => {
    ...
    const labels = Object.keys(ageBins).map(
      (bin) => `${bin}-${parseInt(bin) + binSize - 1}`
    );
  ...
  const UsersByCountryBarChart = ({ filteredUsers }) => {
    ...
    const userCounts = countries.map(
      (country) =>
  ...
  return (
    <div>
      ...
      <div className="admin-page">
        ...
        {users?.length === 0 && (
          <div className="no-data-message">{t("noParticipantsData")}</div>
        )}
